<template>
  <main>
    ...
    <section>
      ...
      <div v-if="LISTE_TOURNOIS_PROCHAIN.length === 0">
        <p>Pas de tournois prévus pour le moment.</p>
      </div>
      <div v-else>
        <div class="card__tournois">
          <ul v-for="t in LISTE_TOURNOIS_PROCHAIN" :key="t.id">
    ...
    <section>
      ...
      <div v-if="LISTE_TOURNOIS_ENCOURS.length === 0">
        <p>Pas de tournois en cours pour le moment.</p>
      </div>
      <div v-else>
        <div class="card__tournois">
          <ul v-for="t in LISTE_TOURNOIS_ENCOURS" :key="t.id">
            <li>nom : {{ t.tournament.name }}</li>
            <li>description : {{ t.tournament.description_source }}</li>
            <li>jeu : {{ t.tournament.game_name }}</li>
            <li>type : {{ t.tournament.tournament_type }}</li>
...
// Donner tournois (pour site de tournois) ------------------------------------------------------------

const env = useRuntimeConfig();

const { data: LISTE_TOURNOIS_PROCHAIN } = await useAsyncData(
  "liste_tournois_pending",
  async () => {
    return $fetch(env.public.challongeApiUrl + "/liste_tournois_pending");
  }
);

const { data: LISTE_TOURNOIS_ENCOURS } = await useAsyncData(
  "liste_tournois_underway",
  async () => {
    return $fetch(env.public.challongeApiUrl + "/liste_tournois_underway");
  }
);
</script>
